...
public class ServiceViewPresenter extends AbstractPresenter<IServiceView> implements IServiceViewPresenter, IServiceEvent {
  ...
  @Override public void setSearchImdb(String imdb) {
  ...
  @Override public void setElapsedTime(long elapsedTime) {
    synchronized (mutex) {
  ...
  @Override public long elapsedTime() {
    synchronized (mutex) {
      return elapsedTime;
    }
  }

  @Override public void notifyElapsedChanged() {
    checkIfTextMatch();
  }

  @Override public void startShow() {
    setUpSession(); //log in user
    resumeShow();
  }

  @Override public void stopShow() {
  ...
  @Override public void onCreate() {
    ...
    eventListener = BusManager.add(new Action1<Object>() {
      @Override public void call(Object event) {
        if (event instanceof HashFoundEvent) {
          onHashFound((HashFoundEvent) event);
        } else if (event instanceof HashNotFoundEvent) {
          onHashNotFound((HashNotFoundEvent) event);
        } else if (event instanceof SequenceFoundEvent) {
          onSequenceFound((SequenceFoundEvent) event);
        } else if(event instanceof SequenceNotFoundEvent) {
          onSequenceNotFound((SequenceNotFoundEvent) event);
        } else if (event instanceof SrtEntityFoundEvent) {
          onSrtEntityFound((SrtEntityFoundEvent) event);
        } else if (event instanceof SrtEntityNotFoundEvent) {
          onSrtEntityNotFound((SrtEntityNotFoundEvent) event);
        }
      }
    });
  }
  ...
  private void notifyUser() {
    ...
    PendingIntent pendingIntent =
        PendingIntent.getService(view.getContext(), 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);

    NotificationCompat.Builder notifyBuilder =
        new NotificationCompat.Builder(view.getContext()).setContentTitle(getString(R.string.text_notification_stop_title))
  ...
  @Override public void onHashNotFound(HashNotFoundEvent hashEvent) {
    ...
    log(Log.ERROR, "we can not get hash for unknown reason...");
    stopShow();//ship has sink :(
  }

  @Override public void onSequenceFound(SequenceFoundEvent sequenceEvent) {
  ...
  @Override public void onSequenceNotFound(SequenceNotFoundEvent sequenceEvent) {
