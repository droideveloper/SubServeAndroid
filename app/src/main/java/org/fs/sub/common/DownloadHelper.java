...
/**
 * Created by Fatih
 * as org.fs.sub.common.DownloadHelper
 */
public final class DownloadHelper implements IDownloadHelper {
    ...
    /**
     * this is the magic method where we download subtitles through the interface and push them into view system
     * @param serviceProxy
     * @param URL
     */
    @Override
    public synchronized void withServiceProxyAndUrlAndHash(IServiceEndpoint serviceProxy,
                                                    String URL, String hash, String lang) {

        subscription = createWithURLAndServiceProxyAndHash(URL, serviceProxy, hash, lang)
    ...
    @Override
    public synchronized void withServiceProxyAndUrlAndImdb(IServiceEndpoint serviceProxy,
                                                           String URL, String imdb, String lang) {

        subscription = createWithURLAndServiceProxyAndImdb(URL, serviceProxy, imdb, lang)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(successCallback(), errorCallback());
    }
     ...
     * @return
     */
    private Observable<SrtEntity> createWithURLAndServiceProxyAndHash(String URL,
                                                                      IServiceEndpoint serviceProxy, String hash, String lang) {
        return Observable.just(new Object[] { URL, serviceProxy, hash, lang })
                .flatMap(new Func1<Object[], Observable<SrtEntity>>() {
                    @Override
                    public Observable<SrtEntity> call(Object[] objects) {
                        try {
    ...
    private Observable<SrtEntity> createWithURLAndServiceProxyAndImdb(String URL,
                                                                      IServiceEndpoint serviceProxy, String imdb, String lang) {
        return Observable.just(new Object[] { URL, serviceProxy, imdb, lang })
                .flatMap(new Func1<Object[], Observable<SrtEntity>>() {
                    @Override
                    public Observable<SrtEntity> call(Object[] objects) {
                        try {
    ...
    private Action1<SrtEntity> successCallback() {
        return new Action1<SrtEntity>() {
            @Override
            public void call(SrtEntity srtEntity) {
                ...
                BusManager.MAIN.post(new SrtEntityFoundEvent(srtEntity, SrtEntityFoundEvent.REMOTE));
            }
        };
    }
     ...
     * @throws IOException
     */
    private SrtEntity parseResponse(Response response) throws IOException {
