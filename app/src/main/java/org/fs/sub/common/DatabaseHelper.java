...
public final class DatabaseHelper extends OrmLiteSqliteOpenHelper implements IDatabaseHelper {
  ...
  @Override public void onUpgrade(SQLiteDatabase database, ConnectionSource connectionSource, int oldVersion,
      int newVersion) {
    try {
  ...
  @Override public void withHashAndFirst(@NonNull String hash) {
    subscription = queryWithHashAndFirst(hash).observeOn(Schedulers.io())
        .subscribeOn(AndroidSchedulers.mainThread())
        .subscribe(actionWithSrtEntitySuccess());
  }

  @Override public void withHashAndLangAndFirst(@NonNull String hash, @NonNull String lang) {
    subscription = queryWithHashAndLangAndFirst(hash, lang).observeOn(Schedulers.io())
        .subscribeOn(AndroidSchedulers.mainThread())
        .subscribe(actionWithSrtEntitySuccess());
  }

  @Override public void withImdbAndLangAndFirst(@NonNull String imdb, @NonNull String lang) {
    subscription = queryWithImdbAndLangAndFirst(imdb, lang).observeOn(Schedulers.io())
        .subscribeOn(AndroidSchedulers.mainThread())
        .subscribe(actionWithSrtEntitySuccess());
  }
  ...
  @Override public void withUpdateOrSaveSrtEntity(@NonNull SrtEntity entity) {
    subscription = saveOrUpdateWithSrtEntity(entity).observeOn(Schedulers.io())
        .subscribeOn(AndroidSchedulers.mainThread())
  ...
  @Override public RuntimeExceptionDao<SrtEntity, Long> getSrtEntityRuntimeDao() {
    ensureDaoCreated();
    return persist;
  }
  ...
  */
  private Observable<SrtEntity> queryWithHashAndFirst(String hash) {
    return Observable.just(hash).flatMap(new Func1<String, Observable<SrtEntity>>() {
      @Override public Observable<SrtEntity> call(String hash) {
        try {
  ...
  private Action1<SrtEntity> actionWithSrtEntitySuccess() {
    return new Action1<SrtEntity>() {
      @Override public void call(final SrtEntity srtEntity) {
        ...
        if (srtEntity != null) {
          handler.post(new Runnable() {
            @Override public void run() {
              BusManager.send(new SrtEntityFoundEvent(srtEntity, SrtEntityFoundEvent.LOCAL));
            }
          });
        } else {
          handler.post(new Runnable() {
            @Override public void run() {
              BusManager.send(new SrtEntityNotFoundEvent(SrtEntityNotFoundEvent.LOCAL));
            }
          });
        }
      }
  ...
  private Observable<SrtEntity> executeQueryWithHashAndFirst(String hash) throws SQLException {
    ensureDaoCreated();
    PreparedQuery<SrtEntity> preparedQuery =
        persist.queryBuilder().where().eq(SrtEntity.CLM_HASH, hash).prepare();
  ...
  private Observable<SrtEntity> queryWithHashAndLangAndFirst(String hash, String lang) {
    return Observable.just(new String[] { hash, lang }).flatMap(new Func1<String[], Observable<SrtEntity>>() {
      @Override public Observable<SrtEntity> call(String[] args) {
        try {
  ...
  private Observable<SrtEntity> queryWithImdbAndLangAndFirst(String imdb, String lang) {
    return Observable.just(new String[] { imdb, lang }).flatMap(new Func1<String[], Observable<SrtEntity>>() {
      @Override public Observable<SrtEntity> call(String[] args) {
        try {
  ...
  private Observable<SrtEntity> executeQueryWithImdbAndLangAndFirst(String imdb, String lang)
      throws SQLException {
    ensureDaoCreated();
    PreparedQuery<SrtEntity> preparedQuery = persist.queryBuilder()
        .where()
        .eq(SrtEntity.CLM_IMDB_ID, imdb)
        .and()
        .eq(SrtEntity.CLM_LANG, lang)
        .prepare();
    SrtEntity srtEntity = persist.queryForFirst(preparedQuery);
    return Observable.just(srtEntity);
  }

  private Observable<SrtEntity> executeQueryWithHashAndLangAndFirst(String hash, String lang)
      throws SQLException {
    ensureDaoCreated();
    PreparedQuery<SrtEntity> preparedQuery = persist.queryBuilder()
        .where()
        .eq(SrtEntity.CLM_HASH, hash)
        .and()
        .eq(SrtEntity.CLM_LANG, lang)
        .prepare();
    SrtEntity srtEntity = persist.queryForFirst(preparedQuery);
  ...
  private Observable<Integer> saveOrUpdateWithSrtEntity(SrtEntity srtEntity) {
    return Observable.just(srtEntity).flatMap(new Func1<SrtEntity, Observable<Integer>>() {
      @Override public Observable<Integer> call(SrtEntity srtEntity) {
        try {
  ...
  private Observable<Integer> executeSaveOrUpdateWithSrtEntity(SrtEntity srtEntity)
      throws SQLException {
    ensureDaoCreated();
  ...
  private Action1<Integer> actionWithSrtEntity() {
    return new Action1<Integer>() {
      @Override public void call(final Integer integer) {
        ...
        handler.post(new Runnable() {
          @Override public void run() {
            BusManager.send(new SrtEntitySavedOrUpdatedEvent(integer));
          }
        });
      }
    };
